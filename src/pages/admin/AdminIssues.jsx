...
const INIT = [
  { id: 1, from: 'Patient', dept: 'Orthopedics', hospital: 'DBH Churu', type: 'Waiting Time', urgency: 'high', status: 'open', date: '2024-11-14', desc: 'Patient waited over 2 hours for consultation.' },
  { id: 2, from: 'Patient', dept: 'Cardiology', hospital: 'DBH Churu', type: 'Staff Behavior', urgency: 'medium', status: 'in-progress', date: '2024-11-13', desc: 'Staff was rude during registration.' },
  { id: 3, from: 'Doctor Head', dept: 'Pediatrics', hospital: 'DBH Churu', type: 'Facility Issue', urgency: 'medium', status: 'open', date: '2024-11-12', desc: 'AC not working in ward.' },
  { id: 4, from: 'Patient', dept: 'ENT', hospital: 'DBH Churu', type: 'Report Delay', urgency: 'low', status: 'resolved', date: '2024-11-11', desc: 'Report not available for 3 days.' },
  { id: 5, from: 'Doctor', dept: 'Neurology', hospital: 'DBH Churu', type: 'Equipment Issue', urgency: 'high', status: 'in-progress', date: '2024-11-10', desc: 'MRI machine calibration required.' },
]
...
export default function AdminIssues() {
  ...
  const [issues, setIssues] = useState(INIT)
  const update = (id) => setIssues(p => p.map(i => i.id === id ? { ...i, status: NEXT[i.status] } : i))

  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h2 className="text-xl font-display font-bold text-gray-800">Issues — District View</h2>
            ...
            <div className="flex items-start justify-between gap-3 flex-wrap">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">{issue.dept} · {issue.hospital} · From: {issue.from} · {issue.date}</p>
                  <p className="text-sm text-gray-600 mt-1">{issue.desc}</p>
                </div>
              </div>
              {issue.status !== 'resolved' && (
                <button onClick={() => update(issue.id)} className="btn-secondary text-xs py-1.5 shrink-0">
